# AI Voice-to-Command 技术规格说明书 (SPEC)
...
- **API 服务**：
    - **STT**：流式语音识别 (WebSocket 协议) 用环境变量里的URL和Api Key，和模型
    - **LLM**：用环境变量里的URL和Api Key，和模型 (追求极速响应)
- **剪贴板**：`pyperclip`
...
你是一个高效的文案整理助手。用户会提供一段口语化的语音转文字记录，请你将其整理为书面、通顺、逻辑清晰的指令或文案。去除“那个”、“然后”、“呃”等冗余词汇，如果有清晰的逻辑关系，可以整理成1,2,3...，之后直接输出整理后的结果，不要进行任何对话或说明。
